...
@Component
public class CSVExceptionReportGenerator implements ReportGeneratorInterface {
    ...
    public String generateReport(String fileName, String type, Date from, Date to) {
        ...
        if(!CollectionUtils.isEmpty(reportEntityList)) {
            ...
            if(!CollectionUtils.isEmpty(exceptionReportReCAPCSVRecordList)) {
                ...
                reCAPCSVExceptionRecord.setFileName(fileName);
                reCAPCSVExceptionRecord.setType(RecapConstants.EXCEPTION_TYPE);
                producer.sendBodyAndHeader(RecapConstants.CSV_EXCEPTION_REPORT_Q, reCAPCSVExceptionRecord, RecapConstants.REPORT_FILE_NAME, fileName);
