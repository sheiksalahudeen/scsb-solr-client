...
public class BibliographicDetailsRepositoryUT extends BaseTestCase {
    ...
    public void updateLastUpdatedDate(){
        List<BibliographicEntity> bibliographicEntityList = saveAndGetBibliographicEntities();
        BibliographicEntity bibliographicEntity = bibliographicEntityList.get(0);
        Date insertedBibHoldingLastUpdatedDate = bibliographicEntity.getBibHoldinglastUpdatedDate();
        Date insertedBibItemLastUpdatedDate = bibliographicEntity.getBibHoldinglastUpdatedDate();
        List<Integer> bibliographicIdList = new ArrayList<>();
        bibliographicIdList.add(bibliographicEntity.getBibliographicId());
        Date lastUpdatedDate = new Date();
        bibliographicDetailsRepository.updateBibItemLastUpdatedDate(bibliographicIdList,"guest",lastUpdatedDate);
        bibliographicDetailsRepository.flush();
        BibliographicEntity fetchedBibliographicEntity = bibliographicDetailsRepository.findByBibliographicId(bibliographicEntity.getBibliographicId());
        assertNotEquals(insertedBibItemLastUpdatedDate,fetchedBibliographicEntity.getBibItemlastUpdatedDate());
    }

    private List<BibliographicEntity> saveAndGetBibliographicEntities() {
